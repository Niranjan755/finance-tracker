...
export function TransactionRow({ transaction, category, account, onClick, showAccount = true }: TransactionRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium">{transaction.merchant || category?.name || 'Transaction'}</p>
        <p className="truncate text-xs text-muted-foreground">
          {category?.name}
          {showAccount && account ? ` · ${account.name}` : ''} · {formatDisplayDate(transaction.date)}
        </p>
      </div>
      <MoneyText
