...
export function UpcomingPage() {
  ...
  const occurrences = useMemo(() => projectAllOccurrences(recurring, today, HORIZON_DAYS), [recurring, today])
  const firstOccurrenceIds = useMemo(() => {
    const seen = new Set<string>()
    return new Set(
      occurrences.filter((o) => (seen.has(o.recurringId) ? false : (seen.add(o.recurringId), true))).map((o) => o.recurringId),
    )
  }, [occurrences])

  const expectedExpenseCents = occurrences.filter((o) => o.type === 'expense').reduce((s, o) => s + o.amountCents, 0)
  const expectedIncomeCents = occurrences.filter((o) => o.type === 'income').reduce((s, o) => s + o.amountCents, 0)
  const totals = computeAccountTotals(accounts)
  const expectedRemainingCents = totals.availableCashCents + expectedIncomeCents - expectedExpenseCents
  ...
  async function handleRecordNow(id: string) {
    try {
      ...
      toast.error('Unable to record transaction', { description: err instanceof Error ? err.message : undefined })
    }
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Card className="p-4">
          <p className="text-xs font-medium text-muted-foreground">Expected Upcoming Expenses</p>
          <MoneyText cents={expectedExpenseCents} kind="expense" signed={false} className="mt-1 block text-2xl font-semibold" />
        </Card>
        <Card className="p-4">
          <p className="text-xs font-medium text-muted-foreground">Expected Remaining Balance</p>
          <MoneyText cents={expectedRemainingCents} kind="neutral" signed={false} className="mt-1 block text-2xl font-semibold" />
        </Card>
      </div>

      <h3 className="mb-3 text-sm font-medium text-muted-foreground">Upcoming Payments</h3>
      {occurrences.length === 0 ? (
        ...
        <Card className="mb-8 divide-y p-1">
          {occurrences.map((o, i) => {
            ...
            const canRecordNow = o.date <= today && firstOccurrenceIds.has(o.recurringId) && occurrences.findIndex((x) => x.recurringId === o.recurringId) === i
            return (
              <div key={`${o.recurringId}-${o.date}`} className="flex items-center gap-3 px-2 py-2.5">
                <span
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    ...
                    {o.date < today && <span className="ml-1 text-amber-600 dark:text-amber-400">Overdue</span>}
                  </p>
                </div>
                <MoneyText cents={o.amountCents} kind={o.type} currency={account?.currency} className="shrink-0 text-sm font-medium" />
                {canRecordNow && (
                  <Button size="sm" variant="outline" onClick={() => handleRecordNow(o.recurringId)}>
                    Record Now
                  </Button>
                )}
              </div>
      ...
      <h3 className="mb-3 text-sm font-medium text-muted-foreground">Recurring Transactions</h3>
      {recurring.length === 0 ? (
        <EmptyState
          icon={CalendarClock}
        ...
        <Card className="divide-y p-1">
          {recurring.map((item) => {
            ...
            const Icon = getIcon(category?.icon ?? 'circle-dashed')
            return (
              <div key={item.id} className={`flex items-center gap-3 px-2 py-2.5 ${!item.isActive ? 'opacity-50' : ''}`}>
                <span
                  className="flex size-9 shrink-0 items-center justify-center rounded-full"
                  style={{ backgroundColor: `${category?.color ?? '#6b7280'}1a`, color: category?.color ?? '#6b7280' }}
                >
                ...
                <MoneyText cents={item.amountCents} kind={item.type} currency={account?.currency} className="shrink-0 text-sm font-medium" />
                <DropdownMenu>
                  <DropdownMenuTrigger
                    render={
                      <Button variant="ghost" size="icon" className="size-8" aria-label={`Actions for ${item.name}`}>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem variant="destructive" onClick={() => setDeletingRecurring(item)}>
                      Delete
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <RecurringForm accounts={accounts} categories={categories} recurring={editingRecurring} onSubmit={handleSubmit} onCancel={() => setFormOpen(false)} />
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deletingRecurring} onOpenChange={(open) => !open && setDeletingRecurring(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the recurring schedule. Transactions already recorded from it are not affected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
