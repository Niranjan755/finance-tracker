    ...
    return (
      <div>
        ...
        <EmptyState
          ...
          description="Add some transactions to see analytics here."
        />
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Card className="p-4">
          <p className="text-muted-foreground text-xs font-medium">Avg Daily Spending</p>
          <MoneyText
            cents={avgDailySpending}
            kind="expense"
            signed={false}
        ...
        <Card className="p-4">
          <p className="text-muted-foreground text-xs font-medium">Avg Monthly Spending</p>
          <MoneyText
            cents={avgMonthlySpending}
            kind="expense"
            signed={false}
            className="mt-1 block text-xl font-semibold"
          />
        ...
        <Card className="p-4">
          <p className="text-muted-foreground text-xs font-medium">Largest Expense</p>
          {largestExpense ? (
            <>
              <MoneyText
                cents={largestExpense.amountCents}
                kind="expense"
                signed={false}
                className="mt-1 block text-xl font-semibold"
              />
              <p className="text-muted-foreground truncate text-xs">
                {largestExpense.merchant || 'Transaction'}
              </p>
            </>
          ) : (
        ...
        <Card className="p-4">
          <p className="text-muted-foreground text-xs font-medium">Most Frequent Merchant</p>
          {topMerchant ? (
            <>
              <p className="mt-1 truncate text-xl font-semibold">{topMerchant.merchant}</p>
              <p className="text-muted-foreground text-xs">
                {topMerchant.transactionCount} transactions
              </p>
            </>
          ) : (
            <p className="mt-1 text-xl font-semibold">-</p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div>
          <h3 className="text-muted-foreground mb-3 text-sm font-medium">Spending by Category</h3>
        ...
        <div>
          <h3 className="text-muted-foreground mb-3 text-sm font-medium">Spending by Account</h3>
          <Card className="space-y-3 p-4">
            {byAccount.map((a) => (
              <div key={a.accountId}>
                <div className="mb-1 flex justify-between text-sm">
                  <span>{a.name}</span>
                  <span className="text-muted-foreground">
                    {formatCurrency(a.amountCents, currency)}
                  </span>
                </div>
        ...
        <div>
          <h3 className="text-muted-foreground mb-3 text-sm font-medium">Income by Source</h3>
          <Card className="space-y-3 p-4">
      ...
      <div className="mt-6">
        <h3 className="text-muted-foreground mb-3 text-sm font-medium">Top Merchants</h3>
          ...
          {merchants.map((m) => (
            <div key={m.merchant} className="flex items-center justify-between px-3 py-2.5 text-sm">
              <div>
                ...
                <p className="text-muted-foreground text-xs">{m.transactionCount} transactions</p>
              </div>
              <MoneyText
                cents={m.totalSpentCents}
                kind="expense"
                signed={false}
                currency={currency}
              />
